...
public class MyPanel extends JPanel implements KeyListener, Runnable {
        ...
        switch (tank.getDirection()) {
            ...
            case 3:
                if (bullet.getX() >= tank.getX() && bullet.getX() <= tank.getX() + 30
                        && bullet.getY() >= tank.getY() && bullet.getY() <= tank.getY() + 15) {
                    bullet.setLive(false);
                    tank.setLive(false);
                }
        }
    ...
    // my bullet hit an enemy tank
    public void myBulletHitTank() {
        for (int i = 0; i < this.myTank.getBullets().size(); i++) {
            Bullet bullet = this.myTank.getBullets().get(i);
    ...
    public void run() {
        while (true) {
            ...
            // judge if my bullets hits enemy tank
            this.myBulletHitTank();

            // judge if enemy bullets hit my tank
            this.hitMyTank();
